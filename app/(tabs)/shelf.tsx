...
export default function ShelfScreen() {
  const { courses, isLoading, error, refresh, updateCourse } =
    useProductivity();
      ...
      >
        <View className="flex-row items-start justify-between">
          <View className="flex-1 pr-4">
            ...
            <Text className="mt-2 text-base text-muted">
              Keep only what deserves your attention.
            </Text>
          </View>
          <Pressable
        ...
        >
          {filters.map((item) => (
            ...
            >
              <Text
                className={`text-sm font-semibold ${filter === item.value ? "text-black" : "text-muted"}`}
              >
                {item.label}
              </Text>
            </Pressable>
          ...
          {visibleCourses.map((course) => {
            ...
            return (
              ...
              >
                ...
                <Text className="mt-6 text-2xl font-bold leading-7 text-cream">
                  {course.title}
                </Text>
        ...
        {!isLoading && visibleCourses.length === 0 ? (
          <View className="mt-8 items-start rounded-[28px] bg-surface p-7">
            ...
            <Text className="mt-2 text-base leading-6 text-muted">
              {filter === "in_progress"
                ? "Add one learning path worth finishing this season."
                : `Courses marked ${filter.replace("_", " ")} will collect here.`}
            </Text>
            {filter === "in_progress" ? (
              <Pressable
