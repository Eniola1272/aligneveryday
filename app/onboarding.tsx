...
export default function OnboardingScreen() {
  ...
  async function submit() {
    if (fullName.trim().length < 2) return setError('Add your name to continue.');
    if (!/^[a-zA-Z0-9_]{3,24}$/.test(username)) {
      return setError('Username must be 3–24 letters, numbers, or underscores.');
    }
  ...
  return (
    ...
    >
      ...
      <FormField label="Your name" onChangeText={setFullName} placeholder="Alex Morgan" value={fullName} />
      <FormField
        autoCapitalize="none"
      ...
      <View className="mb-8 flex-row items-center justify-between rounded-3xl bg-surface p-5">
        <View className="mr-4 flex-1">
          <Text className="font-bold text-cream">Public learning portfolio</Text>
          <Text className="mt-1 text-sm leading-5 text-muted">Let others see completed work—not private tasks.</Text>
        </View>
        <Switch
      ...
      <PrimaryButton isLoading={isSubmitting} label="Enter my workspace" onPress={submit} />
    </AuthScaffold>
  );
}
