...
const promises = [
  [
    "01",
    "Collect",
    "Bring courses and self-directed learning into one focused shelf.",
  ],
  ["02", "Align", "Turn what you are learning into small, finishable actions."],
  [
    "03",
    "Prove",
    "Shape completed work into a portfolio that shows how you think.",
  ],
] as const;
      ...
      <View className="flex-1 px-6 pb-8 pt-8">
        ...
        <View className="mt-12">
          ...
          <Text className="mt-5 max-w-md text-lg leading-7 text-muted">
            A calm operating system for learning, doing, and making your
            progress visible.
          </Text>
        </View>
        ...
        <View className="mt-10 flex-1 justify-center gap-4">
          {promises.map(([number, title, copy]) => (
            <View className="flex-row rounded-3xl bg-surface p-5" key={number}>
              <Text className="mr-4 text-sm font-bold text-accent">
                {number}
              </Text>
